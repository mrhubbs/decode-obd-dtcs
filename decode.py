...
def parse_dtc(raw, _bytes):
    ...
    # cut off the leading 0x and convert to uppercase, in case there are
    # letters present
    form = lambda b : hex(b)[2:].upper()
    ...
    dtc += form(_bytes[0] & 0b1111)  # the last 4 bits of the first byte
    dtc += form(_bytes[1] >> 4)  # the first 4 bits of the second byte
    dtc += form(_bytes[1] & 0b1111)  # the last 4 bits of the second byte

    # pull a description if we have one
    return (dtc, DTC.get(dtc, ""))

for code in codes:
    code_as_bytes = [(code >> 8), code & 0xff]

    result = parse_dtc(code, code_as_bytes)
    print(result)
